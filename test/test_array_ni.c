...
static void check_invalidate_error_on_object_is_null(void **state) {
    ...
    assert_false(seahorse_array_ni_invalidate(NULL));
...
static void check_init_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_init(NULL, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_init_error_on_memory_allocation_failed(void **state) {
    ...
    assert_false(seahorse_array_ni_init(&object, 10));
...
static void check_init(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 10));
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_capacity(void **state) {
    ...
    uintmax_t out;
    assert_true(seahorse_array_ni_capacity(&object, &out));
...
static void check_set_length_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_set_length(NULL, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_set_length_error_on_memory_allocation_failed(void **state) {
    ...
    calloc_is_overridden = true;
    assert_false(seahorse_array_ni_set_length(&object, 1));
    calloc_is_overridden = false;
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_MEMORY_ALLOCATION_FAILED,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
...
static void check_shrink_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_shrink(NULL));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_add_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_add(NULL, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_add_error_on_memory_allocation_failed(void **state) {
    ...
    assert_true(seahorse_array_ni_init(&object, 0));
    calloc_is_overridden = true;
    assert_false(seahorse_array_ni_add(&object, 0));
    calloc_is_overridden = false;
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_MEMORY_ALLOCATION_FAILED,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
...
static void check_add(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_true(seahorse_array_ni_add(&object, rand() % UINTMAX_MAX));
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_add_all_error_on_memory_allocation_failed(void **state) {
    ...
    calloc_is_overridden = true;
    assert_false(seahorse_array_ni_add_all(&object, count, values));
    calloc_is_overridden = false;
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_MEMORY_ALLOCATION_FAILED,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
...
static void check_add_all(void **state) {
    ...
    const uintmax_t count = sizeof(values) / sizeof(uintmax_t);
    assert_true(seahorse_array_ni_add_all(&object, count, values));
    uintmax_t out;
    assert_true(seahorse_array_ni_get_length(&object, &out));
...
static void check_remove_last_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_remove_last(NULL));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove_last_error_on_array_is_null(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_false(seahorse_array_ni_remove_last(&object));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_ARRAY_IS_EMPTY, seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove_last(void **state) {
    ...
    assert_true(seahorse_array_ni_add_all(&object, count, values));
    assert_true(seahorse_array_ni_remove_last(&object));
    uintmax_t out;
    assert_true(seahorse_array_ni_get_length(&object, &out));
...
static void check_insert_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_insert(NULL, 0, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_insert_error_on_index_out_of_bounds(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_false(seahorse_array_ni_insert(&object, 0, 0));
...
static void check_insert(void **state) {
    ...
    assert_true(seahorse_array_ni_insert(&object, 0, 0));
    assert_true(seahorse_array_ni_get_length(&object, &out));
...
static void check_remove_error_on_index_is_out_of_bounds(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_false(seahorse_array_ni_remove(&object, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_INDEX_IS_OUT_OF_BOUNDS,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_remove(void **state) {
    ...
    assert_true(seahorse_array_ni_get_length(&object, &out));
    assert_int_equal(out, 1);
    assert_true(seahorse_array_ni_remove(&object, 0));
    assert_true(seahorse_array_ni_get_length(&object, &out));
...
static void check_remove_all_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_remove_all(NULL, 0, 1));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove_all_error_on_index_is_out_of_bounds(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_false(seahorse_array_ni_remove_all(&object, 0, 10));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_INDEX_IS_OUT_OF_BOUNDS,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_remove_all(void **state) {
    ...
    assert_true(seahorse_array_ni_get_length(&object, &out));
    assert_int_equal(out, count);
    assert_true(seahorse_array_ni_remove_all(&object, 1, 3));
    assert_true(seahorse_array_ni_get_length(&object, &out));
...
static void check_get_error_on_index_is_out_of_bounds(void **state) {
    ...
    assert_true(seahorse_array_ni_init(&object, 0));
    uintmax_t out;
    assert_false(seahorse_array_ni_get(&object, 0, &out));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_INDEX_IS_OUT_OF_BOUNDS,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_get(void **state) {
    ...
    assert_true(seahorse_array_ni_init(&object, 0));
    const uintmax_t check = rand() % UINTMAX_MAX;
    assert_true(seahorse_array_ni_add(&object, check));
    uintmax_t out;
    assert_true(seahorse_array_ni_get(&object, 0, &out));
    assert_int_equal(out, check);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_set_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_array_ni_set(NULL, 0, 0));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_OBJECT_IS_NULL, seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_set_error_on_index_is_out_of_bounds(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_false(seahorse_array_ni_set(&object, 10, 21));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_INDEX_IS_OUT_OF_BOUNDS,
                     seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_set(void **state) {
    ...
    assert_true(seahorse_array_ni_set(&object, 0, check));
    assert_true(seahorse_array_ni_get(&object, 0, &out));
    assert_int_equal(out, check);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_first_error_on_array_is_empty(void **state) {
    ...
    assert_true(seahorse_array_ni_init(&object, 0));
    uintmax_t *out;
    assert_false(seahorse_array_ni_first(&object, &out));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_ARRAY_IS_EMPTY, seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first(void **state) {
    ...
    assert_true(seahorse_array_ni_add(&object, check));
    uintmax_t *out;
    assert_true(seahorse_array_ni_first(&object, &out));
    assert_int_equal(*out, check);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last_error_on_array_is_empty(void **state) {
    ...
    assert_true(seahorse_array_ni_init(&object, 0));
    uintmax_t *out;
    assert_false(seahorse_array_ni_last(&object, &out));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_ARRAY_IS_EMPTY, seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last(void **state) {
    ...
    assert_true(seahorse_array_ni_add(&object, check));
    uintmax_t *out;
    assert_true(seahorse_array_ni_last(&object, &out));
    assert_int_equal(*out, check);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_next_error_on_item_end_of_sequence(void **state) {
    ...
    assert_true(seahorse_array_ni_set_length(&object, 1));
    uintmax_t *item;
    assert_true(seahorse_array_ni_first(&object, &item));
    assert_false(seahorse_array_ni_next(&object, item, &item));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_END_OF_SEQUENCE, seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_next(void **state) {
    ...
    struct seahorse_array_ni object;
    assert_true(seahorse_array_ni_init(&object, 0));
    assert_true(seahorse_array_ni_set_length(&object, 2));
    uintmax_t *item;
    assert_true(seahorse_array_ni_first(&object, &item));
    assert_true(seahorse_array_ni_next(&object, item, &item));
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_prev_error_on_item_end_of_sequence(void **state) {
    ...
    assert_true(seahorse_array_ni_set_length(&object, 1));
    uintmax_t *item;
    assert_true(seahorse_array_ni_last(&object, &item));
    assert_false(seahorse_array_ni_prev(&object, item, &item));
    assert_int_equal(SEAHORSE_ARRAY_NI_ERROR_END_OF_SEQUENCE, seahorse_error);
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_prev(void **state) {
    ...
    uintmax_t *item;
    assert_true(seahorse_array_ni_last(&object, &item));
    assert_true(seahorse_array_ni_prev(&object, item, &item));
    assert_true(seahorse_array_ni_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
