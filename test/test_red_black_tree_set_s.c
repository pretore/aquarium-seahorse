...
static void
check_init_red_black_tree_set_s_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_set_s_init_red_black_tree_set_s(
            NULL, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_SET_S_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void
check_init_red_black_tree_set_s_error_on_other_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_set_s_init_red_black_tree_set_s(
            (void *) 1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_SET_S_ERROR_OTHER_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void check_init_red_black_tree_set_s(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    size_t out;
    ...
    assert_true(seahorse_red_black_tree_set_s_init(&object));
    assert_true(seahorse_red_black_tree_set_s_add(&object, &value));
    struct seahorse_red_black_tree_set_s copy;
    assert_true(seahorse_red_black_tree_set_s_init_red_black_tree_set_s(
            &copy, &object));
    assert_true(seahorse_red_black_tree_set_s_invalidate(&object));
    const struct sea_turtle_string *item;
    assert_true(seahorse_red_black_tree_set_s_get(&copy, &value, &item));
    assert_ptr_not_equal(item, &value);
    assert_int_equal(sea_turtle_string_compare(item, &value), 0);
    assert_true(seahorse_red_black_tree_set_s_invalidate(&copy));
    assert_true(sea_turtle_string_invalidate(&value));
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void
check_init_red_black_tree_set_s_error_on_memory_allocation_failed(
        void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    size_t out;
    const char chars[] = u8"copy initialize";
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_init_red_black_tree_set_s_error_on_object_is_null),
            cmocka_unit_test(check_init_red_black_tree_set_s_error_on_other_is_null),
            cmocka_unit_test(check_init_red_black_tree_set_s),
            cmocka_unit_test(check_init_red_black_tree_set_s_error_on_memory_allocation_failed),
    };
