...
static void check_invalidate_error_on_object_is_null(void **state) {
    ...
    assert_false(seahorse_red_black_tree_map_ni_i_invalidate(NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_init_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_init(NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_init(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_i object;
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_count_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_count(NULL, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_count_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_count((void *) 1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_count(void **state) {
    ...
    uintmax_t count;
    assert_true(seahorse_red_black_tree_map_ni_i_count(&object, &count));
...
static void check_add_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_add(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_add_error_on_value_is_null(void **state) {
    ...
    assert_false(seahorse_red_black_tree_map_ni_i_add(
            (void *) 1, 0, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_VALUE_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_add(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, key, &value));
    assert_true(seahorse_red_black_tree_map_ni_i_count(&object, &count));
...
static void check_add_error_on_key_already_exists(void **state) {
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, key, &value));
    assert_false(seahorse_red_black_tree_map_ni_i_add(&object, key, &value));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_ALREADY_EXISTS,
                     seahorse_error);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_remove_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_remove(NULL, 0));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove_error_on_key_not_found(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    const uintmax_t key = rand() % UINTMAX_MAX;
    assert_false(seahorse_red_black_tree_map_ni_i_remove(&object, key));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_count(&object, &count));
    assert_int_equal(count, 1);
    assert_true(seahorse_red_black_tree_map_ni_i_remove(&object, key));
    assert_true(seahorse_red_black_tree_map_ni_i_count(&object, &count));
    assert_int_equal(count, 0);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_contains_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_contains(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_contains(void **state) {
    ...
    const uintmax_t key = rand() % UINTMAX_MAX;
    bool result;
    assert_true(seahorse_red_black_tree_map_ni_i_contains(
            &object, key, &result));
    assert_false(result);
    struct sea_turtle_integer value;
    assert_true(sea_turtle_integer_init_uintmax_t(
...
static void check_get_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_get(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_get_error_on_key_not_found(void **state) {
    ...
    assert_false(seahorse_red_black_tree_map_ni_i_get(&object, key, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_get(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, key, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_get(&object, key, &out));
...
static void check_ceiling_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_ceiling(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_ceiling(void **state) {
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_ceiling(&object, 10, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    assert_true(seahorse_red_black_tree_map_ni_i_ceiling(
            &object, 11, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_invalidate(&value));
...
static void check_ceiling_error_on_memory_allocation_failed(void **state) {
    ...
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = true;
    assert_false(seahorse_red_black_tree_map_ni_i_ceiling(&object, 10, &out));
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = false;
    assert_int_equal(
            SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MEMORY_ALLOCATION_FAILED,
...
static void check_floor_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_floor(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_floor(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_floor(&object, 100, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    assert_true(seahorse_red_black_tree_map_ni_i_floor(&object, 19, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_floor_error_on_memory_allocation_failed(void **state) {
    ...
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = true;
    assert_false(seahorse_red_black_tree_map_ni_i_floor(&object, 100, &out));
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = false;
    assert_int_equal(
            SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MEMORY_ALLOCATION_FAILED,
...
static void check_higher_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_higher(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_higher_error_on_key_not_found(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    struct sea_turtle_integer *out;
    assert_false(seahorse_red_black_tree_map_ni_i_higher(&object, 100, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_false(seahorse_red_black_tree_map_ni_i_higher(&object, 101, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_higher(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_higher(&object, 10, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_higher_error_on_memory_allocation_failed(void **state) {
    ...
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = true;
    assert_false(seahorse_red_black_tree_map_ni_i_higher(&object, 10, &out));
    malloc_is_overridden = calloc_is_overridden = realloc_is_overridden
            = posix_memalign_is_overridden = false;
    assert_int_equal(
            SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MEMORY_ALLOCATION_FAILED,
...
static void check_lower_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_lower(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_lower_error_on_key_not_found(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    struct sea_turtle_integer *out;
    assert_false(seahorse_red_black_tree_map_ni_i_lower(&object, 100, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_false(seahorse_red_black_tree_map_ni_i_lower(&object, 99, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_lower(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_lower(&object, 100, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_first_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_first(NULL, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_first((void *) 1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first_error_on_map_is_empty(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    struct sea_turtle_integer *out;
    assert_false(seahorse_red_black_tree_map_ni_i_first(&object, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MAP_IS_EMPTY,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_first(&object, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
...
static void check_last_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_last(NULL, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_last((void *) 1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last_error_on_map_is_empty(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    struct sea_turtle_integer *out;
    assert_false(seahorse_red_black_tree_map_ni_i_last(&object, &out));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MAP_IS_EMPTY,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_last(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_last(&object, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
...
static void check_get_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_get_entry(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_get_entry_error_on_key_not_found(void **state) {
    ...
    struct seahorse_red_black_tree_map_ni_i object;
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_false(seahorse_red_black_tree_map_ni_i_get_entry(
            &object, rand() % UINTMAX_MAX, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_get_entry(void **state) {
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(
            &object, key, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_get_entry(
            &object, key, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_ptr_not_equal(out, &key);
    assert_int_equal(key, *(uintmax_t *) out);
    assert_true(seahorse_red_black_tree_map_ni_i_entry_value(
...
static void check_ceiling_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_ceiling_entry(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_ceiling_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(
            &object, 10, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_ceiling_entry(
            &object, 10, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *) out, 10);
    assert_true(seahorse_red_black_tree_map_ni_i_entry_value(
            &object, entry, (struct sea_turtle_integer **) &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *) out, 100);
    assert_true(seahorse_red_black_tree_map_ni_i_entry_value(
            &object, entry, (struct sea_turtle_integer **) &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
...
static void check_floor_entry_error_on_key_not_found(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_false(seahorse_red_black_tree_map_ni_i_floor_entry(
            &object, rand() % UINTMAX_MAX, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_floor_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(
            &object, 100, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_floor_entry(
            &object, 100, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
...
static void check_higher_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_higher_entry(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_higher_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(
            &object, 100, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_higher_entry(
            &object, 10, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
...
static void check_higher_entry_error_on_memory_allocation_failed(void **state) {
    srand(time(NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_i object;
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
...
static void check_lower_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_lower_entry(
            NULL, 0, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_lower_entry_error_on_key_not_found(void **state) {
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 101, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_false(seahorse_red_black_tree_map_ni_i_lower_entry(
            &object, 101, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_KEY_NOT_FOUND,
...
static void check_lower_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_lower_entry(
            &object, 100, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
...
static void check_first_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_first_entry(NULL, (void*)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first_entry_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_first_entry((void*)1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_first_entry_error_on_map_is_empty(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_false(seahorse_red_black_tree_map_ni_i_first_entry(&object, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MAP_IS_EMPTY,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_first_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_first_entry(&object, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *)out, 100);
...
static void check_last_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_last_entry(NULL, (void*)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last_entry_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_last_entry((void*)1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_last_entry_error_on_map_is_empty(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_false(seahorse_red_black_tree_map_ni_i_last_entry(&object, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_MAP_IS_EMPTY,
                     seahorse_error);
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_last_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_last_entry(&object, &entry));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *)out, 10);
...
static void check_remove_entry_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_remove_entry(
            NULL, (void *)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_remove_entry_error_on_entry_is_null(void **state) {
    ...
    assert_false(seahorse_red_black_tree_map_ni_i_remove_entry(
            (void *)1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_ENTRY_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_remove_entry(void **state) {
    ...
    assert_true(seahorse_red_black_tree_map_ni_i_remove_entry(&object, entry));
    assert_true(seahorse_red_black_tree_map_ni_i_count(&object, &count));
...
static void check_next_entry_error_on_entry_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_next_entry(NULL, (void *)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_ENTRY_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_next_entry_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_next_entry((void *)1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_next_entry_error_on_end_of_sequence(void **state) {
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(
            &object, rand() % UINTMAX_MAX, &value));
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_first_entry(&object, &entry));
    assert_false(seahorse_red_black_tree_map_ni_i_next_entry(entry, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_END_OF_SEQUENCE,
                     seahorse_error);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_next_entry(void **state) {
    ...
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 100, &value));
    assert_true(seahorse_red_black_tree_map_ni_i_next_entry(entry, &entry));
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *)out, 100);
    assert_true(seahorse_red_black_tree_map_ni_i_entry_value(
...
static void check_prev_entry_error_on_entry_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_prev_entry(NULL, (void *)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_ENTRY_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_prev_entry_error_on_out_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_prev_entry((void *)1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OUT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}
...
static void check_prev_entry_error_on_end_of_sequence(void **state) {
    ...
    const struct seahorse_red_black_tree_map_ni_i_entry *entry;
    assert_true(seahorse_red_black_tree_map_ni_i_last_entry(&object, &entry));
    assert_false(seahorse_red_black_tree_map_ni_i_prev_entry(entry, &entry));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_END_OF_SEQUENCE,
                     seahorse_error);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
...
static void check_prev_entry(void **state) {
    ...
    assert_true(sea_turtle_integer_set_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, 10, &value));
    assert_true(seahorse_red_black_tree_map_ni_i_prev_entry(entry, &entry));
    assert_true(seahorse_red_black_tree_map_ni_i_entry_key(
            &object, entry, (const uintmax_t **) &out));
    assert_int_equal(*(uintmax_t *)out, 10);
    assert_true(seahorse_red_black_tree_map_ni_i_entry_value(
...
static void check_entry_key_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_entry_key(
            NULL, (void *)1, (void *)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void check_entry_value_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_entry_value(
            NULL, (void *)1, (void *)1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void
check_init_red_black_tree_map_ni_i_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_init_red_black_tree_map_ni_i(
            NULL, (void *) 1));
    assert_int_equal(
            SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OBJECT_IS_NULL,
            seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void
check_init_red_black_tree_map_ni_i_error_on_other_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_i_init_red_black_tree_map_ni_i(
            (void *) 1,  NULL));
    assert_int_equal(
            SEAHORSE_RED_BLACK_TREE_MAP_NI_I_ERROR_OTHER_IS_NULL,
            seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void check_init_red_black_tree_map_ni_i(void **state) {
    srand(time(NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_i object;
    assert_true(seahorse_red_black_tree_map_ni_i_init(&object));
    ...
    assert_true(sea_turtle_integer_init_uintmax_t(
            &value, rand() % UINTMAX_MAX));
    assert_true(seahorse_red_black_tree_map_ni_i_add(&object, key, &value));
    struct seahorse_red_black_tree_map_ni_i copy;
    assert_true(seahorse_red_black_tree_map_ni_i_init_red_black_tree_map_ni_i(
            &copy, &object));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&object));
    struct sea_turtle_integer *out;
    assert_true(seahorse_red_black_tree_map_ni_i_get(&copy, key, &out));
    assert_ptr_not_equal(out, &value);
    assert_int_equal(sea_turtle_integer_compare(&value, out), 0);
    assert_true(sea_turtle_integer_invalidate(&value));
    assert_true(seahorse_red_black_tree_map_ni_i_invalidate(&copy));
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void
check_init_red_black_tree_map_ni_i_error_on_memory_allocation_failed(
        void **state) {
    srand(time(NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_i object;
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_init_red_black_tree_map_ni_i_error_on_object_is_null),
            cmocka_unit_test(check_init_red_black_tree_map_ni_i_error_on_other_is_null),
            cmocka_unit_test(check_init_red_black_tree_map_ni_i),
            cmocka_unit_test(check_init_red_black_tree_map_ni_i_error_on_memory_allocation_failed),
    };
