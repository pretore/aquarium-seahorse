...
static void check_entry_key(void **state) {
    ...
    struct seahorse_red_black_tree_map_ni_p object;
    assert_true(seahorse_red_black_tree_map_ni_p_init(&object));
    const uintmax_t key = (rand() % UINT8_MAX);
    void *value = (void *)(uintptr_t) (rand() % UINTMAX_MAX);
    assert_true(seahorse_red_black_tree_map_ni_p_add(
            &object, key, value));
    const struct seahorse_red_black_tree_map_ni_p_entry *entry;
...
static void
check_init_red_black_tree_map_ni_p_error_on_object_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_p_init_red_black_tree_map_ni_p(
            NULL, (void *) 1));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_P_ERROR_OBJECT_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
...
static void
check_init_red_black_tree_map_ni_p_error_on_other_is_null(void **state) {
    seahorse_error = SEAHORSE_ERROR_NONE;
    assert_false(seahorse_red_black_tree_map_ni_p_init_red_black_tree_map_ni_p(
            (void *) 1, NULL));
    assert_int_equal(SEAHORSE_RED_BLACK_TREE_MAP_NI_P_ERROR_OTHER_IS_NULL,
                     seahorse_error);
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void check_init_red_black_tree_map_ni_p(void **state) {
    srand(time(NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_p object;
    assert_true(seahorse_red_black_tree_map_ni_p_init(&object));
    ...
    const void *value = (const void *) (uintptr_t) (rand() % UINTMAX_MAX);
    assert_true(seahorse_red_black_tree_map_ni_p_add(&object, key, value));
    struct seahorse_red_black_tree_map_ni_p copy;
    assert_true(seahorse_red_black_tree_map_ni_p_init_red_black_tree_map_ni_p(
            &copy, &object));
    assert_true(seahorse_red_black_tree_map_ni_p_invalidate(&object, NULL));
    const void *out;
    assert_true(seahorse_red_black_tree_map_ni_p_get(&copy, key, &out));
    assert_ptr_equal(out, value);
    assert_true(seahorse_red_black_tree_map_ni_p_invalidate(&copy, NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
}

static void
check_init_red_black_tree_map_ni_p_error_on_memory_allocation_failed(
        void **state) {
    srand(time(NULL));
    seahorse_error = SEAHORSE_ERROR_NONE;
    struct seahorse_red_black_tree_map_ni_p object;
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_init_red_black_tree_map_ni_p_error_on_object_is_null),
            cmocka_unit_test(check_init_red_black_tree_map_ni_p_error_on_other_is_null),
            cmocka_unit_test(check_init_red_black_tree_map_ni_p),
            cmocka_unit_test(check_init_red_black_tree_map_ni_p_error_on_memory_allocation_failed),
    };
