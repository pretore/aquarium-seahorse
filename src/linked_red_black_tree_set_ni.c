...
static void init(struct seahorse_linked_red_black_tree_set_ni *const object) {
    assert(object);
...
bool seahorse_linked_red_black_tree_set_ni_init_linked_red_black_tree_set_ni(
        struct seahorse_linked_red_black_tree_set_ni *const object,
        const struct seahorse_linked_red_black_tree_set_ni *const other) {
    if (!object) {
        seahorse_error =
                SEAHORSE_LINKED_RED_BLACK_TREE_SET_NI_ERROR_OBJECT_IS_NULL;
        return false;
    ...
    if (!other) {
        seahorse_error =
                SEAHORSE_LINKED_RED_BLACK_TREE_SET_NI_ERROR_OTHER_IS_NULL;
        return false;
    }
    init(object);
    const uintmax_t *item;
    if (!seahorse_linked_red_black_tree_set_ni_first(other, &item)) {
    ...
    do {
        if (!seahorse_linked_red_black_tree_set_ni_add(object, *item)) {
            seagrass_required_true(
    ...
    } while (seahorse_linked_red_black_tree_set_ni_next(other, item, &item));
    seagrass_required_true(
